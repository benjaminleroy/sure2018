...
#The LabourOrSex composite variable
glm_comp_list <- mno_regression(data_list[["data_corrected"]][["training"]], 
                           imp_formula_comp, glm, family = "binomial")

glm_comp_interaction_list <- mno_regression(data_list[["data_corrected"]][["training"]], 
                           imp_formula_comp_interaction, glm, family = "binomial")
...
#and getting approximating the variance in the parameters due to imputation
labour_interaction_betas_variance <- apply(betas, MARGIN = 2, var)
...
#and getting approximating the variance in the parameters due to imputation
sex_betas_variance <- apply(betas, MARGIN = 2, var)
...
#and getting approximating the variance in the parameters due to imputation
sex_interaction_betas_variance <- apply(betas, MARGIN = 2, var)
...
Looking at the Composite Variable model
```{r}
##The model without interactions
comp_betas <- beta_matrix(glm_comp_list)

#Visualizing the beta matrix
vis_beta_mat(comp_betas)

#and getting approximating the variance in the parameters due to imputation
comp_betas_variance <- apply(comp_betas, MARGIN = 2, var)

#build ROC Curves for each model
roc_comp_list <- list()
for(i in 1:length(glm_comp_list)){
  roc_comp_list[[i]] <-
    pROC::roc(data_list[["data_corrected"]][["test"]][[i]][["LabourOrSex"]],
                        predict(glm_comp_list[[i]], type = "response", 
                                newdata = data_list[["data_corrected"]][["test"]][[i]]))
}
...
##The model with interaction 
comp_interaction_betas <- beta_matrix(glm_comp_interaction_list)

#Visualizing the beta matrix
vis_beta_mat(comp_interaction_betas)

#and getting approximating the variance in the parameters due to imputation
comp_interaction_betas_variance <- apply(betas, MARGIN = 2, var)

#build ROC Curves for each model
roc_comp_interaction_list <- list()
for(i in 1:length(glm_comp_interaction_list)){
  roc_comp_interaction_list[[i]] <-
    pROC::roc(data_list[["data_corrected"]][["test"]][[i]][["LabourOrSex"]],
                        predict(glm_comp_interaction_list[[i]], type = "response", 
                                newdata = data_list[["data_corrected"]][["test"]][[i]]))
}
...
save(step_labour_interaction, paste())
```
